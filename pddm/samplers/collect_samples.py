        ...
        for step_num in range(steps_per_rollout):
            ...
            if self.is_random:
                action, _ = self.policy.get_action(
                    observation, prev_action, self.random_sampling_params
                )
            else:
            ...
            if visualize:
                render_env(
                    self.env
                )  # this render being on slows down the timing below, but that's ok

        if visualize:
            print(
                "    1 rollout, time taken: {:0.4f} s".format(time.time() - starttime)
            )
