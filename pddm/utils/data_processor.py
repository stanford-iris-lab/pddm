...
class DataProcessor:
    def __init__(
        self, params, duplicateData_switchObjs=False, indices_for_switching=[]
    ):
    ...
    def update_stats(self, model, dataset_trainRand, dataset_trainOnPol):
        ...
        if dataset_trainOnPol.dataX.shape[0] == 0:
            ...
            temp_y = np.concatenate([dataset_trainRand.dataY, dataset_trainOnPol.dataY])
            temp_z = np.concatenate([dataset_trainRand.dataZ, dataset_trainOnPol.dataZ])
        ...
        self.normalization_data.mean_x = np.mean(np.concatenate(temp_x, 0), axis=0)
        self.normalization_data.std_x = np.std(np.concatenate(temp_x, 0), axis=0)

        self.normalization_data.mean_y = np.mean(np.concatenate(temp_y, 0), axis=0)
        self.normalization_data.std_y = np.std(np.concatenate(temp_y, 0), axis=0)
    ...
    def preprocess_data(self, dataset):
        ...
        if dataset.dataX.shape[0] > 0:
            x_preprocessed = (
                dataset.dataX - self.normalization_data.mean_x
            ) / self.normalization_data.std_x
            y_preprocessed = (
                dataset.dataY - self.normalization_data.mean_y
            ) / self.normalization_data.std_y
            z_preprocessed = (
                dataset.dataZ - self.normalization_data.mean_z
    ...
    def xyz_to_disc_io(self, dataset_preprocessed_real, dataset_preprocessed_pred):

        if dataset_preprocessed_real.dataX.shape[0] > 0:
            inputs_real = np.concatenate(
                (dataset_preprocessed_real.dataX, dataset_preprocessed_real.dataY, dataset_preprocessed_real.dataZ), axis=1
            )
            outputs_real = np.ones(dataset_preprocessed_real.dataX.shape[0])


            inputs_pred = np.concatenate(
                (dataset_preprocessed_pred.dataX, dataset_preprocessed_pred.dataY, dataset_preprocessed_pred.dataZ), axis=1
            )
            outputs_pred = np.zeros(dataset_preprocessed_pred.dataX.shape[0])

            inputs = np.concatenate([inputs_real, inputs_pred], axis = 0)
            outputs = np.concatenate([outputs_real, outputs_pred], axis = 0)

            # TODO: suffle indices
            print("WARNING: IMPLEMENT SHUFFLE INDICES")
            return inputs, outputs
        else:
            return np.array([]), np.array([])
    ...
    def update_model_normalization(self, model):
        ...
        if type(model) == list:
            for model_indiv in model:
                model_indiv.normalization_data = copy.deepcopy(self.normalization_data)
        else:
            model.normalization_data = copy.deepcopy(self.normalization_data)

    def convertTransitionsToDatasets(self, transitions):
        """Convert (s,a,s')s to a dataset of ((s), (a), (s'))"""
        ...
        for transition in transitions: 
            ...
            all_differences.append(transition.difference)
            

        # training labels:
        dataZ = np.array(all_differences)

        # turn the list of rollouts into large arrays of data
        dataX = self.array_datatype(np.array(all_states))
        dataY = self.array_datatype(np.array(all_actions))
        ...
        if self.duplicateData_switchObjs:
            ...
            # switch for dataX
            dataX_second[:, :, obj_start1 : obj_start1 + 6] = dataX_temp[
                :, :, obj_start2 : obj_start2 + 6
            ]
            dataX_second[:, :, obj_start2 : obj_start2 + 6] = dataX_temp[
                :, :, obj_start1 : obj_start1 + 6
            ]
            dataX_second[:, :, target_start1 : target_start1 + 2] = dataX_temp[
                :, :, target_start2:
            ]
            dataX_second[:, :, target_start2:] = dataX_temp[
                :, :, target_start1 : target_start1 + 2
            ]

            # switch for dataZ
            dataZ_second[:, obj_start1 : obj_start1 + 6] = dataZ_temp[
                :, obj_start2 : obj_start2 + 6
            ]
            dataZ_second[:, obj_start2 : obj_start2 + 6] = dataZ_temp[
                :, obj_start1 : obj_start1 + 6
            ]
            dataZ_second[:, target_start1 : target_start1 + 2] = dataZ_temp[
                :, target_start2:
            ]
            dataZ_second[:, target_start2:] = dataZ_temp[
                :, target_start1 : target_start1 + 2
            ]
