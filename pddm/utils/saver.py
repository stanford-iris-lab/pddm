...
class Saver:
    ...
    def save_initialData(self, params, rollouts_trainRand, rollouts_valRand):
        ...
        pickle.dump(
            rollouts_valRand,
            open(self.save_dir + "/training_data/val_rollouts_rand.pickle", "wb"),
            pickle.HIGHEST_PROTOCOL,
        )
        pickle.dump(
            params, open(self.save_dir + "/params.pkl", "wb"), pickle.HIGHEST_PROTOCOL
        )
    ...
    def save_model(self):
        ...
        save_path1 = self.tf_saver.save(
            self.sess,
            self.save_dir + "/models/model_aggIter" + str(self.iter_num) + ".ckpt",
        )
    ...
    def save_training_info(self, save_data):
        ...
        pickle.dump(
            ...
            open(
                self.save_dir
                + "/training_data/train_rollouts_onPol_iter"
                + str(self.iter_num)
                + ".pickle",
                "wb",
            ),
        ...
        pickle.dump(
            ...
            open(
                self.save_dir
                + "/training_data/val_rollouts_onPol_iter"
                + str(self.iter_num)
                + ".pickle",
                "wb",
            ),
        ...
        pickle.dump(
            ...
            open(
                self.save_dir
                + "/training_data/normalization_data_"
                + str(self.iter_num)
                + ".pickle",
                "wb",
            ),
        ...
        np.save(
            self.save_dir + "/losses/list_training_loss.npy", save_data.training_losses
        )
        np.save(self.save_dir + "/datapoints_per_agg.npy", save_data.training_numData)

        # train/val losses from model training
        np.save(
            self.save_dir
            + "/losses/training_losses_iter"
            + str(self.iter_num)
            + ".npy",
        ...
        np.save(
            self.save_dir
            + "/losses/validation_losses_xaxis_iter"
            + str(self.iter_num)
            + ".npy",
        ...
        np.save(
            self.save_dir
            + "/losses/validation_onPol_losses_iter"
            + str(self.iter_num)
            + ".npy",
            save_data.training_lists_to_save["val_loss_list_onPol"],
        )
        np.save(
            self.save_dir + "/losses/old_losses_iter" + str(self.iter_num) + ".npy",
            save_data.training_lists_to_save["rand_loss_list"],
        )
        np.save(
            self.save_dir + "/losses/new_losses_iter" + str(self.iter_num) + ".npy",
    ...
    def save_rollout_info(self, save_data):
        ...
        np.save(
            self.save_dir + "/rollouts_rewardsPerIter.npy",
            save_data.rollouts_rewardsPerIter,
        )
        np.save(
            self.save_dir + "/rollouts_scoresPerIter.npy",
        ...
        plot_mean_std(scores[:, 0], scores[:, 1], self.save_dir + "/scores_perIter")
