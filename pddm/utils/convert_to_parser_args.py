...
def convert_to_parser_args(args_source=sys.argv[1:]):
    ...
    # model training
    parser.add_argument(
        "--always_use_savedModel", action="store_true"
    )  # use saved model instead of training one
    parser.add_argument(
        "--batchsize", type=int, default=500
    )  # batchsize for each gradient step
    parser.add_argument("--lr", type=float, default=0.001)  # learning rate
    parser.add_argument("--nEpoch", type=int, default=40)  # epochs of training
    parser.add_argument(
        "--nEpoch_init", type=int, default=40
    )  # epochs of training for 1st iter
    ...
    parser.add_argument(
        "--num_control_samples", type=int, default=700
    )  # number of candidate ac sequences
    parser.add_argument(
        "--controller_type", type=str, default="mppi"
    )  # rand, cem, mppi

    # cem
    parser.add_argument("--cem_max_iters", type=int, default=3)  # number of iters
    parser.add_argument(
        "--cem_num_elites", type=int, default=20
    )  # elites for refitting sampling dist

    # mppi
    parser.add_argument("--mppi_kappa", type=float, default=1.0)  # reward weighting
    parser.add_argument(
        "--mppi_mag_noise", type=float, default=0.9
    )  # magnitude of sampled noise
