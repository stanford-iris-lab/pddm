...
class CubeEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(
        ...
        xml_path = os.path.join(
            os.path.dirname(__file__), "assets", "shadowhand_hand_cube.xml"
        )
        ...
        super().__init__(
            ...
            camera_settings=dict(distance=0.9, azimuth=-40, elevation=-50,),
        )
    ...
    def get_reward(self, observations, actions):
        ...
        self.reward_dict["r_total"] = (
            self.reward_dict["ori_dist"] + self.reward_dict["drop_penalty"]
        )
    ...
    def get_score(self, obs_dict):
        return -1.0 * np.linalg.norm(
            obs_dict["object_orientation"] - obs_dict["desired_orientation"]
        )
    ...
    def _get_obs(self):
        ...
        self.obs_dict["object_position"] = qp_obj[
            :3
        ].copy()  # 3 (x/y/z) translation pos of cube
        self.obs_dict["object_orientation"] = qp_obj[
            3:6
        ].copy()  # 3 (r/p/y) rotational pos of cube
        ...
        self.obs_dict[
            "desired_orientation"
        ] = self.desired_pose.copy()  # 3 desired (r/p/y) orientation of cube
