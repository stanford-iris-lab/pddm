...
class Robot(object):
    def __init__(
        self, n_jnt, n_obj, n_dofs, pos_bounds=None, vel_bounds=None, **kwargs
    ):
    ...
    def get_obs(self, env, robot_noise_ratio=0.05, object_noise_ratio=0.05):
        ...
        if not env.initializing:
            noise_amp = robot_noise_ratio * (
                env.model.jnt_range[: self.n_jnt, 1]
                - env.model.jnt_range[: self.n_jnt, 0]
            )
            qp += noise_amp * env.np_random.uniform(low=-0.5, high=0.5, size=self.n_jnt)
            qv += noise_amp * env.np_random.uniform(low=-0.5, high=0.5, size=self.n_jnt)
            if self.has_obj:
                noise_amp = object_noise_ratio * (
                    env.model.jnt_range[-self.n_obj :, 1]
                    - env.model.jnt_range[-self.n_obj :, 0]
                )
                qp_obj += noise_amp * env.np_random.uniform(
                    low=-0.5, high=0.5, size=self.n_obj
                )
                qv_obj += noise_amp * env.np_random.uniform(
                    low=-0.5, high=0.5, size=self.n_obj
                )
    ...
    def enforce_velocity_limits(self, ctrl_position, step_duration):
        ...
        desired_vel = (
            ctrl_position[: self.n_jnt] - last_obs.qpos_robot[: self.n_jnt]
        ) / step_duration

        feasible_vel = np.clip(
            desired_vel,
            self.vel_bounds[: self.n_jnt, 0],
            self.vel_bounds[: self.n_jnt, 1],
        )
