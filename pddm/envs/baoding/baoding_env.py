    ...
    def __init__(
        ...
        if self.which_task == Task.MOVE_TO_LOCATION:
            xml_path = os.path.join(
                os.path.dirname(__file__), "assets", "shadowhand_baoding_1visible.xml"
            )
        else:
            xml_path = os.path.join(
                os.path.dirname(__file__), "assets", "shadowhand_baoding_2.xml"
            )
        ...
        super().__init__(
            ...
            camera_settings=dict(distance=0.7, azimuth=-60, elevation=-50,),
        )
    ...
    def get_reward(self, observations, actions):
        ...
        if self.which_task == Task.MOVE_TO_LOCATION:
            self.reward_dict["r_total"] = (
                5 * self.reward_dict["pos_dist_1"] + self.reward_dict["drop_penalty"]
            )
        else:
            self.reward_dict["r_total"] = (
                5 * self.reward_dict["pos_dist_1"]
                + 5 * self.reward_dict["pos_dist_2"]
                + self.reward_dict["drop_penalty"]
                + self.reward_dict["wrist_angle"]
            )
